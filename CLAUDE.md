...
- **`IPtalkManager`** (`Models/IPtalk/IPtalkManager.swift` + `Models/IPtalk/IPtalkProtocol.swift`) — real IPtalk-compatible UDP client (栗田茂明氏作 IPtalk). Opens 6 `NWListener`s per channel (display 6711, monitor 6712, correction 6713, member-reply 6718, member-broadcast 6722, undo 6723; channel N adds 100×(N-1)). Text is plain Shift-JIS terminated by LF — no custom header. `IPtalkProtocol.swift` holds the pure functions (port arithmetic, encode/decode, member-discovery payloads, sender-host canonicalization, broadcast-address derivation) and is the place to iterate on wire format if real-IPtalk packet captures reveal the Phase 1 payload guesses are wrong. `IPtalkManager.swift` owns the listener stack and the broadcast destination. Finalized speech-recognition lines auto-publish via `TranscriptionManager.onFinalizedLine` when `iptalkAutoSend` (UserDefault) is true.

  Connection ownership is the subtle part. Network.framework does **not** retain connections for you: one reachable only from its own `stateUpdateHandler` can deallocate (and force-cancel) before reaching `.ready`, silently dropping the datagram — so the manager owns every live connection in `inboundConnections` / `outboundConnections` and releases it on completion. Accepted flows keep their receive armed instead of cancelling after one datagram, because a peer that reuses its source port sends many over a single flow. That in turn requires reaping: a UDP flow never reports its own end, and a peer using a fresh source port per datagram — this app does — mints one flow per line. `inboundEvictionKeys` is the pure eviction policy (idle `inboundIdleTimeout`, then LRU under `maxInboundConnections` — instance properties defaulting to 60s and 64) and is where to test that behaviour.

  Two rules govern addressing. Every arrival is keyed through `IPtalkProtocol.canonicalHost`, because a dual-stack peer otherwise registers twice in the member list — once as `192.168.1.5`, once as the IPv4-mapped `::ffff:192.168.1.5` — and a link-local peer splits again by interface zone (`%en0`). And broadcasts go to a subnet-directed address derived from the live interface list (`en*` first, loopback and link-local skipped), not to `255.255.255.255`; since the stack may refuse that destination and nothing tells us in advance, the first send failure retries once on `255.255.255.255` and pins it for the session (`broadcastFallbackEngaged`, cleared on reconnect).
